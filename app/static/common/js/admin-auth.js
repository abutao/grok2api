...
async function getStoredAppKey() {
  ...
  if (!stored) return '';
  try {
    const raw = (await decryptAppKey(stored)) || '';
    const normalized = raw.trim().startsWith('Bearer ')
      ? raw.trim().slice(7).trim()
      : raw.trim();
    if (normalized && normalized !== raw) {
      await storeAppKey(normalized);
    }
    return normalized;
  } catch (e) {
    clearStoredAppKey();
...
async function verifyKey(url, key) {
  ...
  const normalized = raw.startsWith('Bearer ') ? raw.slice(7).trim() : raw;
  const headers = normalized ? { 'Authorization': `Bearer ${normalized}` } : {};
  const res = await fetch(url, { method: 'GET', headers });
  return res.ok;
}

async function ensureAdminKey(options = {}) {
...
function buildAuthHeaders(apiKey) {
  ...
  if (raw.startsWith('Bearer ')) {
    const normalized = raw.replace(/^Bearer\s+Bearer\s+/i, 'Bearer ');
...
async function fetchStorageType() {
  const apiKey = await ensureAdminKey({ redirect: false });
