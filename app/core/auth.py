...
from app.core.storage import CONFIG_FILE
...
def is_public_enabled() -> bool:
    ...
    是否开启 public 功能入口。
    """
    raw_value = get_config("app.public_enabled", None)
    if raw_value is None:
        file_value = _read_public_enabled_from_file()
        if file_value is None:
            return bool(DEFAULT_PUBLIC_ENABLED)
        return _to_bool(file_value)
...
def _read_public_enabled_from_file():
    if not CONFIG_FILE.exists():
        return None
    try:
        content = CONFIG_FILE.read_bytes()
        data = tomllib.loads(content.decode("utf-8"))
        return data.get("app", {}).get("public_enabled", None)
    except Exception:
        return None
